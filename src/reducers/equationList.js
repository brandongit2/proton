function processInput(raw, lastTypedChar = '', caretPos = 0) {
    ...
    newCaretPos += 1;
    if (!raw.substring(caretPos + 1).match(/^\w/u)) {
        switch (lastTypedChar) {
            ...
            case ']':
                ...
                break;
            case '|':
                console.log(raw.charAt(caretPos + 1));
                if (raw.charAt(caretPos + 1) === '|') {
                    newRaw = raw.substring(0, caretPos) + raw.substring(caretPos + 1);
                } else {
                    newRaw += '|';
...
function rawToKatex([raw, caretPos]) {
    ...
    // Place cursor
    katex = `${katex.substring(0, caretPos)}\`bigm\`\`vert\` ${katex.substring(caretPos)}`;
    console.log(caretPos, katex);
    ...
    console.log(katex);
    katex = katex.replace(/`(.*?)`/gu, String.raw`\$1`);
...
function equationList(state = {}, action) {
    switch (action.type) {
        ...
        case 'BACKSPACE': {
            ...
            return {
                ...
                [action.id]: {
                    ...state[action.id],
                    raw:      newValue,
                    katex:    rawToKatex(processInput(newValue, '', caretPos - numCharsForDeletionBehind))[1],
        ...
        case 'MOVE_CARET': {
            let curr = state[action.id];
            return {
                ...state,
                [action.id]: {
                    ...state[action.id],
                    katex:    rawToKatex(processInput(curr.raw, '', action.position - 1))[1],
